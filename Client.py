...
class SMTPClientSide:
    ...
    def evaluate_state(self, end_of_file: bool = False) -> bool:
        ...
        if self.state == self.EXPECTING_SERVER_HELLO:
            ...
            # At this point, we need to start going through the "forward file lines" and generate
            # SMTP commands
        ...
        if self.state == self.EXPECTING_MAIL_FROM:
            ...
            if socket_send_msg(self.connection_socket, self.parser.generate_mail_from_cmd(), self.debug_mode):
                self.advance_forward_file_line_pointer()
            return True

        if self.state == self.EXPECTING_RCPT_TO:
            # print(self.parser.generate_rcpt_to_cmd())
            if socket_send_msg(self.connection_socket, self.parser.generate_rcpt_to_cmd(), self.debug_mode()):
                self.advance_forward_file_line_pointer()
            return True
